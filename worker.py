...
def process_webhook_in_background(webhook_id, headers, body):
    ...
    try:
        ...
        # --- 2. Notify clients via Redis pub/sub ---
        print("[Background] Publishing WebSocket notification via Redis")
        try:
            notification = {
            ...
            conn.publish('webhook_events', json.dumps(notification))
            print(f"[Background] Published notification: {notification}")
        except Exception as e:
            print(f"[Background] Error publishing notification: {e}")
